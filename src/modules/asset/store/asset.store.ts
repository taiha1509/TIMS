...
class AssetModule extends VuexModule {
    ...
    assetsGroupByAssignee: IAssetsGroupByAssignee[] = [];
    ...
    @Action
    async getAssetsGroupByAssignee() {
        const response = await assetService.getAssetsGroupByAssignee({
            ...this.assetQueryString,
        });
        if (response.success) {
            this.MUTATE_TOTAL_ASSIGNEES(response?.data?.totalItems || 0);
            this.MUTATE_ASSET_GROUP_BY_ASSIGNEE(response?.data?.items || []);
        } else {
            this.MUTATE_TOTAL_ASSIGNEES(0);
            this.MUTATE_ASSET_GROUP_BY_ASSIGNEE([]);
        }
    }
    ...
    @Action
    setAssetQueryString(query: IQueryStringAsset) {
    ...
    @Mutation
    MUTATE_TOTAL_ASSIGNEES(totalAssignees: number) {
    ...
    @Mutation
    MUTATE_ASSET_GROUP_BY_ASSIGNEE(assetsGroupByAssignee: IAssetsGroupByAssignee[]) {
        this.assetsGroupByAssignee = assetsGroupByAssignee;
    }
}
