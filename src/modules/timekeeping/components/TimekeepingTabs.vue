...
export default class TimeKeepingTabs extends mixins(TimeKeepingMixins) {
    ...
    async handleChangeTab(tabName: string): Promise<void> {
        ...
        if (tabName === this.timekeepingOptions.WEEK) {
            ...
            startDate = moment().startOf('week').add(1, 'day').utc().fmFullTimeString();
            endDate = moment().endOf('week').add(1, 'day').endOfDay().fmFullTimeString();
            timeKeepingModule.setTimeKeepingFilter({
                selectedWeek: moment().startOf('week').fmFullTimeString(),
            });
        } else {
        ...
        timeKeepingModule.setTimeKeepingQueryString({
            ...
            startDate,
            endDate,
            keyword: initTimeKeepingQueryString.keyword,
        });
